...
def objective_optuna(trial: "optuna.trial.Trial", args) -> float:
    ...
    elapsed = run_trial_and_get_elapsed(
        ...
        tau=tau,
        mu=mu,
        base_work_dir=args.run_dir / "hyperopt",
...
def main(cli_args: dict | None = None):
    ...
    parser.add_argument("--gap-pct", type=float, default=0.0025)
    ...
    args.run_dir = Path(args.work_dir).resolve() / args.run_id

    study = optuna.create_study(direction="minimize", study_name=args.study_name)
    func = lambda t: objective_optuna(t, args)
    print(
        f"Starting optimization: study={args.study_name}, n_trials={args.n_trials}, "
    ...
    with (out_dir / f"optuna_summary_{args.study_name}_{args.run_id}.json").open("w", encoding="utf-8") as f:
...
#  --n-trials 10 --max-workers 2 --gurobi-threads-per-bundle 1